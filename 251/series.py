...
def basic_series() -> pd.Series:
    ...
    """
    ser = pd.Series([1, 2, 3, 4, 5], name='Fred')
...
def alpha_index_series() -> pd.Series:
    ...
    data = {num: letters[num - 1] for num in range(1, len(letters) + 1)}
    flipped_dict = {v : k for k, v in data.items()}
    ser = pd.Series(data=flipped_dict, index=flipped_dict.keys())
...
def object_values_series() -> pd.Series:
    ...
    Don't worry about the series name.
    """
    indices = list(range(101, 127))
    ser = pd.Series(data=list(string.ascii_uppercase), index=indices)
