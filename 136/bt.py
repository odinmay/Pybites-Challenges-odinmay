...
def get_num(bloodtype):
    """Return int from any bloodtype input"""
    if isinstance(bloodtype, int):
        if bloodtype > 7 or bloodtype < 0:
            raise ValueError
        else:
            bt = bloodtype
            return bt

    if isinstance(bloodtype, str):
        try:
            bt = blood_type_text[bloodtype].value
            return bt
        except:
            raise ValueError

    if isinstance(bloodtype, Bloodtype):
        bt = bloodtype.value
        return bt
...
def check_bt(donor, recipient):
    ...
    """
    donor = get_num(donor)
    recipient = get_num(recipient)
    compatible = []
    for num in _particular_antigen_comp(donor, recipient):
        if num >= 0:
...
print(check_bt(Bloodtype.A_NEG, Bloodtype.A_NEG))
